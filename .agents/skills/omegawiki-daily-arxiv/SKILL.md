---
name: omegawiki-daily-arxiv
description: "Run or manage the daily arXiv recommendation feed. Use for one-off fresh-paper recommendations, scheduled GitHub Actions setup/status/disable, email digests, and explicit high-confidence auto-ingest through $omegawiki-ingest."
...
# $omegawiki-daily-arxiv

> Run or manage the daily paper recommendation feed. Bare `$omegawiki-daily-arxiv` means "run today's recommendation pass now"; GitHub Actions is only the unattended scheduler for the same pipeline.
...
- `$omegawiki-daily-arxiv`: run a one-off recommendation pass now. If `config/daily-arxiv.yml` is missing, infer defaults from the wiki and continue.
- `$omegawiki-daily-arxiv setup`: create or repair `config/daily-arxiv.yml` from `config/daily-arxiv.yml.example`; ensure `.github/workflows/daily-arxiv.yml` exists and that its `daily-arxiv:` job's `env:` block exposes both `SEMANTIC_SCHOLAR_API_KEY` and `DEEPXIV_TOKEN` — **add the missing lines automatically** rather than asking the user to hand-edit YAML (without these exposures the prepare step rate-limits out, identical symptom to never setting the secrets); and explain required secrets. See *Setup Workflow* below for the exact patch procedure.
- `$omegawiki-daily-arxiv status`: inspect config, workflow presence, schedule, mode, API/e-mail secret availability, and recent artifacts when available.
- `$omegawiki-daily-arxiv disable`: set `schedule.enabled: false` in config or tell the user what to change; manual `$omegawiki-daily-arxiv` must still work.
...
Triggered by `$omegawiki-daily-arxiv setup`. Idempotent — re-running on a healthy repo is a no-op.
...
3. If mode is `auto-ingest`, use Codex runtime only: choose `decision: ingest` + `confidence: high`, obey `max_auto_ingest`, and invoke `$omegawiki-ingest <arxiv-url>` sequentially. Do not hand-write wiki or graph files. Third-party LLMs are recommendation-only and must not auto-ingest.
...
Writes only scratch files under `.daily-arxiv/` during inform runs. In `auto-ingest`, all durable wiki/raw mutations must come from `$omegawiki-ingest`.

## Relationships

- `$omegawiki-discover` answers deliberate next-read requests from anchors, topics, or wiki state; it never ingests.
- `$omegawiki-daily-arxiv` watches the fresh arXiv stream and can notify daily or manually.
- `$omegawiki-ingest` is the only paper incorporation path. `$omegawiki-daily-arxiv` may call it only in explicit `auto-ingest` mode.
