...
- `--linked-idea <idea-slug>` (optional): explicit binding when the positional arg is free text but the user already has an idea page they want every new experiment to reference. Equivalent to passing the slug positionally; provided so the SPA can call `$omegawiki-exp-design --linked-idea <slug>` from the idea reader page.
...
Decompose the idea's `## Hypothesis` along three dimensions. The output of this step is a tabular scope sheet, not new wiki pages — `$omegawiki-exp-design` does not create concepts or methods. (If the scoping reveals a missing concept, the right move is `$omegawiki-edit` or `$omegawiki-ingest`, not silent creation here.)
...
1. **Create experiment pages**:
   ...
   ```
   Create `wiki/experiments/{slug}.md` following `runtime/schema/entities.yaml::experiments` and `runtime/templates/experiments.md.tmpl` exactly — every frontmatter field below must be present even if empty, because `$omegawiki-exp-run` later uses `tools/research_wiki.py set-meta` to update them, and `set-meta` refuses to create fields that don't already exist:
   ```yaml
   ---
   title: ""
   slug: ""
   ...
   metrics: []
   baseline: ""
   outcome: ""                  # empty until $omegawiki-exp-run Phase 4 — succeeded | failed | inconclusive
   key_result: ""               # empty until $omegawiki-exp-run Phase 4
   date_planned: YYYY-MM-DD
   date_completed: ""           # empty until $omegawiki-exp-run Phase 4
   run_log: ""                  # empty until $omegawiki-exp-run Phase 2
   started: ""                  # empty until $omegawiki-exp-run Phase 2 (ISO timestamp, set via set-meta)
   estimated_hours: 0           # 0 until $omegawiki-exp-run Phase 2 (set via set-meta)
   remote:                      # full block must exist so $omegawiki-exp-run --env remote can populate sub-fields via Edit
     server: ""
     gpu: ""
     session: ""
     started: ""
   ...
   ## Results
   (to be filled after $omegawiki-exp-run)

   ## Analysis
   (to be filled after $omegawiki-exp-run)

   ## Idea updates
   (to be filled after $omegawiki-exp-eval — records the linked idea's status / pilot_result transition)
...
2. **Add graph edges**:
   ...
   python3 tools/research_wiki.py add-edge wiki/ \
     ...
     --type tested_by --evidence "Designed by $omegawiki-exp-design"
   ```
...
7. **Print EXPERIMENT_PLAN_REPORT to terminal**:
   ...
   ## Next Steps
   - Run `$omegawiki-exp-run [[baseline-slug]]` to start Stage 1
   - After each stage, run `$omegawiki-exp-eval` to update the linked idea's status / pilot_result
   ```

## Constraints

- **Every experiment must reference an idea**: `linked_idea` is required by the schema and by this skill's contract. If no idea page exists, refuse to design experiments — instruct the user to run `$omegawiki-ideate` or write an idea page first.
...
- **Scoped propositions are not persisted**: the dimension table from Step 2 is a planning artifact, not a wiki write. Do not create new concept/method/topic pages from `$omegawiki-exp-design`.
...
- **Free-text input without `--linked-idea`**: refuse to proceed — direct the user to `$omegawiki-ideate` first or to pass `--linked-idea`.
...
- **Wiki is empty**: proceed normally but baseline experiments have no prior results to reference; recommend running `$omegawiki-ingest` for relevant papers first.
...
### Shared References
- `.agents/skills/shared-references/cross-model-review.md` — Step 5 Review LLM review independence (if enabled)

### Called by
- `$omegawiki-research` Stage 2 (experiment design stage)
- User directly
- The SPA's idea-page action button (`$omegawiki-exp-design --linked-idea <idea-slug>` via `tools/serve.py`)
