...
- Reads: `config/codex-setup-guide.md` (reference for what each key does)
...
Read `config/codex-setup-guide.md` to load the complete reference for all configurable keys,
...
================================
✓  OPENAI_API_KEY      — managed by Codex (codex login)
...
Used by $omegawiki-ingest, $omegawiki-init, $omegawiki-novelty, $omegawiki-ideate. Free to get.
**Recommended** — without it, $omegawiki-init runs 3x slower and citation-chain expansion is much less effective."
...
and trending paper detection. Used by $omegawiki-daily-arxiv, $omegawiki-novelty, $omegawiki-ideate, $omegawiki-ingest, $omegawiki-init.
...
adversarial review. It's used by $omegawiki-review, $omegawiki-novelty, $omegawiki-ideate, $omegawiki-paper-plan, $omegawiki-paper-draft,
$omegawiki-rebuttal, $omegawiki-refine, $omegawiki-exp-eval, $omegawiki-exp-design, and $omegawiki-daily-arxiv inform recommendations.
Works with any OpenAI-compatible API.
Without it, those skills skip the cross-model review step (everything still works)."

**Present the provider table** from `config/codex-setup-guide.md` (Key 3 section).
...
After the user finishes configuring, run the verification check from `config/codex-setup-guide.md`:
...
and that the user can run `$omegawiki-setup` again anytime to add them.
...
Configuration done. Next:
  ...
  • $omegawiki-init and direct local $omegawiki-ingest will manage generated inputs under raw/discovered/ and raw/tmp/
  • Run: $omegawiki-init [your-research-topic]
```
...
- **`.env` not found**: Inform the user that `setup-codex.sh` was not run yet. Offer to create `.env` from `.env.example`:
  ```bash
  cp config/.env.example .env
  ```
  Then continue with configuration.

- **`config/codex-setup-guide.md` not found**: Proceed using the information in this SKILL.md directly.
...
### Files Read
- `config/codex-setup-guide.md` — complete reference for all configurable keys
