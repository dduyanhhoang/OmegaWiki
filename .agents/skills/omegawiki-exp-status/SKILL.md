...
>
> When used with `$omegawiki-research --auto`, acts as a periodic checker scheduled by CronCreate:
> when all experiments in a pipeline are completed, automatically triggers `$omegawiki-research --start-from stage4`.
...
- `--collect-ready` (optional): auto-call `$omegawiki-exp-run --collect` for all experiments whose session has already ended
- `--auto-advance` (optional, requires `--pipeline <slug>`): if all pipeline experiments are `completed`,
  automatically trigger `$omegawiki-research --start-from stage4` without waiting for the user
...
- `wiki/outputs/pipeline-progress.md` — `--auto-advance` updates current_stage → stage4 (done internally by $omegawiki-research --start-from stage4)
...
### Writes
- `wiki/experiments/{slug}.md` — updated via $omegawiki-exp-run --collect in `--collect-ready` mode
- `wiki/outputs/pipeline-progress.md` — updated by $omegawiki-research when `--auto-advance` triggers Stage 4
- `wiki/log.md` — appended status check log

### Graph edges created
- None (result writes triggered indirectly via $omegawiki-exp-run --collect do not produce new edges)
...
1. **Determine check scope**:
   - If `--pipeline <slug>` is specified:
     ...
     - If the file does not exist or slug does not match: report error, suggest running `$omegawiki-research` first or specifying manually
   - Otherwise:
...
2. **If no running experiments**:
   - Print a friendly message:
     ...
     - To start an experiment: $omegawiki-exp-run <slug>
...
|-----------|---------|-------|--------|
| [[exp-foo-ablation]] | 0.8h | NaN loss at step 500 | Run `$omegawiki-exp-run exp-foo-ablation --collect` to inspect |
...
$omegawiki-exp-status --collect-ready

# Collect a specific experiment:
$omegawiki-exp-run exp-foo-sanity --collect

# Pipeline progress (if in $omegawiki-research):
$omegawiki-exp-status --pipeline {pipeline-slug}
```
```
...
For each `completed_pending_collect` experiment, call `$omegawiki-exp-run --collect`:
...
- **Anomalies are not auto-fixed**: `$omegawiki-exp-status` only reports anomalies; fixes require the user to manually call `$omegawiki-exp-run --collect`
...
- **`--pipeline` but pipeline-progress.md does not exist**: report error "Pipeline progress file not found. Run `$omegawiki-research <direction>` first or check wiki/outputs/"
...
- **`$omegawiki-exp-run --collect` fails** (`--collect-ready` mode): record the failure, continue collecting other experiments, report all failures at the end

## Dependencies

### Skills（via Skill tool）
- `$omegawiki-exp-run` — call collect phase in `--collect-ready` mode
- `$omegawiki-research` — trigger Stage 4 via `--auto-advance`
...
- `Skill` — call $omegawiki-exp-run --collect and $omegawiki-research

### Called by
- CronCreate schedule (created by `$omegawiki-research --auto` Stage 3b: triggers every 30 minutes)
- User directly
- `$omegawiki-research` Stage 3b (in interactive mode, suggested to user)
