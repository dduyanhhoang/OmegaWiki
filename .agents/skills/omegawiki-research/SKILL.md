...
> **Zero-friction entry**: if the wiki is empty, Bootstrap is triggered automatically (search + auto-ingest 5 papers); no need to run $omegawiki-init manually.
...
> When all experiments finish, Stage 4 is triggered automatically. Use `$omegawiki-exp-status` at any time to check progress.
>
...
- `--start-from <stage>` (optional): resume execution from the specified stage
  ...
  - `stage3-check`: check experiment status only (equivalent to `$omegawiki-exp-status --pipeline {slug}`), do not continue execution
  - Requires `wiki/outputs/pipeline-progress.md` to exist
- `--skip-paper` (optional): run research only (Stages 1-4), skip paper writing (Stage 5), but still run $omegawiki-exp-eval (Stage 4)
- `--venue` (optional): target conference (ICLR / NeurIPS / ICML / ACL / CVPR), passed to $omegawiki-paper-plan
...
- `wiki/graph/open_questions.md` — knowledge gaps (passed to $omegawiki-ideate)
...
### Graph edges created
- None directly — all graph edges are delegated to sub-skills ($omegawiki-ideate, $omegawiki-exp-design, $omegawiki-exp-eval each create their own edges)
...
2. **Auto-recovery detection** (when `--start-from` is not specified):
   - If `wiki/outputs/pipeline-progress.md` exists and `status == running`:
     ...
     - Use AskUserQuestion to prompt the user:
       ...
       [3] View experiment status first ($omegawiki-exp-status --pipeline {slug})
       ```
     ...
     - If user selects [3]: call `$omegawiki-exp-status --pipeline {slug}` then exit without continuing
...
3. **Check recovery** (when `--start-from` is specified):
   ...
   - **`--start-from stage3-check`**: equivalent to calling `$omegawiki-exp-status --pipeline {slug}`; display status then exit
...
Deploy each experiment in run order (baseline → validation → ablation → robustness) by calling `$omegawiki-exp-run {experiment_slug}` (default deploy mode, Phase 1+2):
...
4. Output instructions then **end current session**:
   ...
   Recommended time to return: Tomorrow 10:00+
     ...
     $omegawiki-exp-status                              ← confirm all experiments complete
     $omegawiki-research --start-from stage3-collect    ← collect results and continue
...
**Trigger**: user manually runs `$omegawiki-research --start-from stage3-collect`
...
Call `$omegawiki-exp-eval` for each completed experiment:
...
2. Call `$omegawiki-refine` to improve the experiment plan:
   ```
   Skill: refine
...
Call sub-skills in sequence: $omegawiki-paper-plan → $omegawiki-paper-draft → $omegawiki-refine → $omegawiki-paper-compile

**5a. Call $omegawiki-paper-plan**:
```
Skill: paper-plan
Args: "{linked_idea_slugs}" --venue {venue}
```
(passes the validated idea slug(s) collected in Stage 4 to $omegawiki-paper-plan)

**5b. Call $omegawiki-paper-draft**:
```
Skill: paper-draft
Args: "wiki/outputs/PAPER_PLAN.md" --review
```

**5c. Call $omegawiki-refine on paper**:
```
Skill: refine
Args: "paper/main.tex" --max-rounds 3 --target-score 8 --focus writing
```

**5d. Call $omegawiki-paper-compile**:
```
Skill: paper-compile
Args: "paper/"
```
...
- **--skip-paper still runs Stage 4 $omegawiki-exp-eval**: idea/experiment updates must be completed even when not writing a paper
...
- **Stage 3c baseline collect fails**: terminate pipeline; report baseline cannot be reproduced; suggest re-running $omegawiki-exp-design
...
- **User selects view status (auto-recovery detection [3])**: call `$omegawiki-exp-status --pipeline {slug}` then exit without starting a new pipeline

## Dependencies

### Skills（via Skill tool）
- `$omegawiki-ingest` — Stage 0 Bootstrap auto-ingest
- `$omegawiki-ideate` — Stage 1 idea discovery
- `$omegawiki-exp-design` — Stage 2 experiment design
- `$omegawiki-exp-run` — Stage 3a (deploy mode) and Stage 3c (--collect mode)
- `$omegawiki-exp-status` — user manually checks experiment progress; `--auto-advance` can automatically trigger Stage 4 when all complete
- `$omegawiki-exp-eval` — Stage 4 verdict
- `$omegawiki-refine` — Stage 4 iteration + Stage 5 paper improvement
- `$omegawiki-paper-plan` — Stage 5 paper planning
- `$omegawiki-paper-draft` — Stage 5 paper writing
- `$omegawiki-paper-compile` — Stage 5 paper compilation
