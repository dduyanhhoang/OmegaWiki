...
- `wiki/outputs/PAPER_PLAN.md` — understand paper structure (from $omegawiki-paper-plan, if --paper-slug provided)
- `wiki/graph/context_brief.md` — global context
- `wiki/graph/edges.jsonl` — idea-experiment-paper-method relationships
- `.agents/skills/shared-references/cross-model-review.md` — Review LLM stress-test independence
...
- Propose a concrete supplementary experiment plan (can link to $omegawiki-exp-design)
...
**Follow cross-model-review.md**: do not send Codex's rebuttal strategy analysis to Review LLM.
...
→ Run `$omegawiki-exp-design ablation-dataset-x` to design follow-up
...
- **Do not silently flip a linked idea's status**: rebuttal only flags concerns by appending to ideas' `## Risks` / `## Lessons learned` or methods' `## Limitations`; status transitions are reserved for `$omegawiki-exp-eval`
...
- **Wiki empty**: warn that wiki knowledge base is empty; suggest running $omegawiki-ingest to populate ideas, methods, and experiments
...
### Shared References
- `.agents/skills/shared-references/cross-model-review.md` — Review LLM stress-test independence principle

### Suggested follow-up skills
- `$omegawiki-exp-design` — design supplementary experiments for concerns with insufficient evidence
- `$omegawiki-paper-draft` — prepare revised paper (based on Paper Edits checklist)
