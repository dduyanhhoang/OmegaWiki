...
> Can be used standalone or called by $omegawiki-ideate Phase 4.
...
- `--write` (optional, default **off**): persist the resulting `novelty_score` to the target's frontmatter. **Only takes effect when `target` is an idea slug** (i.e. `wiki/ideas/{slug}.md` exists). Free-text targets and paper-novelty checks remain read-only regardless of this flag. Treat as a user-owned flag — `$omegawiki-ideate` Phase 4 sets it explicitly when calling `$omegawiki-novelty`; do not infer it from repo state.
...
- Take the lower of Codex's search-based score and Review LLM's score (conservative principle)
...
- **Review LLM independence**: do not include Codex's own novelty judgment when submitting to Review LLM; let Review LLM assess independently
...
### Shared References
- `.agents/skills/shared-references/cross-model-review.md` (created in Phase 2, Review LLM independence principle)
