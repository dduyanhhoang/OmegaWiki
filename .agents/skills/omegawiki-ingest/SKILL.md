...
Turn one paper into a fully wired set of wiki pages. Emit well-formed entities and correct cross-references; leave semantic audits (backlink symmetry, dangling nodes, field-value policing) for `$omegawiki-check`.
...
- `references/dedup-policy.md` — merge-vs-create decision rule for concepts and methods, and the line that separates `$omegawiki-ingest` shape checks from `$omegawiki-check` semantic audits
- `references/cross-references.md` — forward/reverse link matrix and paper-to-paper edge-type selection
- `references/init-mode.md` — manifest-driven handoff from `$omegawiki-init` and parallel-safety conventions
...
- `source`: one of — arXiv URL (e.g. `https://arxiv.org/abs/2106.09685`), local `.tex`, local `.pdf`, or a `canonical_ingest_path` handed off by `$omegawiki-init` via `.checkpoints/init-sources.json`(see `references/init-mode.md`)
- `--discover` (optional, default **off**): after the final report, invoke `$omegawiki-discover --anchor <this-paper's-arxiv-id>` and append the shortlist to the report as "Related papers you may want to ingest next". Never auto-ingests the suggestions. Skipped automatically in INIT MODE. Treat this as a user-owned flag: do not set it based on repo state.
- `--visualize` (optional, default **off**): after Step 7 rebuild, regenerate Canvas visualization artifacts via `tools/visualize.py generate-canvas`. Skipped automatically in INIT MODE — the parent `$omegawiki-init` handles visualization once at fan-in. Treat this as a user-owned flag: do not set it based on repo state. (The interactive web Graph view lives in the SPA at `app/modules/graph.js`, served by `tools/serve.py`; it reads `wiki/graph/` live and needs no per-ingest regeneration.)
...
- `wiki/topics/{slug}.md` — EDIT only (no CREATE from `$omegawiki-ingest`)
...
1. If `$omegawiki-init` passed a `canonical_ingest_path`, enter **INIT MODE** and consume that path verbatim. Do not rescan `raw/`. See `references/init-mode.md`.
...
The shape check is intentionally narrow. Backlink symmetry, dangling-node detection, and cross-entity consistency are `$omegawiki-check`'s job, not this skill's.
...
Skip this whole step in INIT MODE — the parent `$omegawiki-init` handles it at fan-in.
...
- Surface unmatched high-citation references in the final report so the user can decide whether to follow up with another `$omegawiki-ingest`.
...
2. Do not create new topic pages from `$omegawiki-ingest` — topic creation belongs to `$omegawiki-init` and `$omegawiki-edit`.
...
Skip this step unless the user explicitly passed `--visualize`. Also skip it in INIT MODE — `$omegawiki-init`'s parent process regenerates Canvas + HTML once at fan-in, so individual subagents must not duplicate the work and risk concurrent writes.

When active, regenerate Canvas + HTML (best-effort; visualize failure must not fail `$omegawiki-ingest`):

```bash
"$PYTHON_BIN" tools/visualize.py generate-canvas wiki/ \
  || echo "WARN: visualize generate-canvas failed; run $omegawiki-visualize manually" >&2
```

`--obsidian` is not regenerated here — `wiki/.obsidian/graph.json` is project-level static config that only changes when `config/visualize.json` palette changes; run `$omegawiki-visualize --obsidian` manually for that case.
...
Skip this step unless the user explicitly passed `--discover`. Also skip it in INIT MODE — `$omegawiki-init`'s parent process decides whether to run discovery at fan-in, not individual subagents.

When active, invoke `$omegawiki-discover` with the just-ingested paper as the single anchor:
...
Append the markdown output to the report under a heading like "Related papers you may want to ingest next". Do not auto-ingest anything from the shortlist — the user picks. If discovery fails (S2 outage, all channels empty), note the failure in one line and continue — a failed `$omegawiki-discover` must not fail an otherwise successful `$omegawiki-ingest`.

## Constraints

- `raw/papers/`, `raw/notes/`, `raw/web/` are user-owned and read-only. Direct local `$omegawiki-ingest` may add prepared sidecars under `raw/tmp/`; direct arXiv ingests may write fetched source artifacts under `raw/discovered/`. INIT MODE treats all of `raw/` as read-only.
...
- In INIT MODE, do not write reverse links into pages that already exist (created by a sibling worktree or scaffold). Record the relationship via `tools/research_wiki.py add-edge` only; the parent `$omegawiki-init` backfills reverse links during fan-in.
...
- Ingest is conservative about new entities:
  ...
  - Any further candidates must be merged into their nearest existing entry, or left out for `$omegawiki-check` to flag. Rationale and matching rules: `references/dedup-policy.md`.
...
- `$omegawiki-ingest` runs a shape check on its own output (required keys, enum ranges, YAML parses) and stops there. Backlink symmetry, dangling nodes, and full semantic audits belong to `$omegawiki-check`. Do not re-implement them here.
- Assume another `$omegawiki-ingest` may run concurrently in a sibling worktree. All shared-file writes (`graph/edges.jsonl`, `graph/citations.jsonl`, `index.md`, `log.md`) must go through `tools/research_wiki.py` or use append-only semantics. See `references/init-mode.md`.
- In INIT MODE, skip `fetch_s2.py citations`, `fetch_s2.py references`, and the `rebuild-*` commands — the parent `$omegawiki-init` runs them once after fan-in.
- In INIT MODE, also skip Step 7.5 visualization regardless of whether `--visualize` was set; the parent `$omegawiki-init` regenerates Canvas + HTML once at fan-in to avoid concurrent writes from sibling worktrees.
...
- `.agents/skills/shared-references/citation-verification.md`

### Skills

- `$omegawiki-init` — calls `$omegawiki-ingest` in parallel subagents via INIT MODE
- `$omegawiki-check` — audits wiki state after `$omegawiki-ingest` completes; owns every semantic check `$omegawiki-ingest` intentionally does not perform
- `$omegawiki-discover` — optional follow-up when `--discover` is set; produces a shortlist of related papers the user may want to ingest next
- `$omegawiki-visualize` — Step 7.5 (when `--visualize` is set and not in INIT MODE) regenerates Canvas + HTML by calling `tools/visualize.py` directly (best-effort)
