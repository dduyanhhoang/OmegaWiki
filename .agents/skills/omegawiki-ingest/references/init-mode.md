# $omegawiki-ingest INIT MODE and Parallel Safety

Open this reference when `$omegawiki-ingest` is invoked by `$omegawiki-init` as a parallel subagent, or any time you need to understand what concurrent ingests may be doing to shared files.

## When INIT MODE is active

INIT MODE is active for any `$omegawiki-ingest` invocation whose source path originates from `.checkpoints/init-sources.json`. The parent `$omegawiki-init` runs one `$omegawiki-ingest` per paper in an isolated `git worktree`, following the contract in `skills/init/references/parallel-ingest.md`.

In INIT MODE:

- the source is always a `canonical_ingest_path` already prepared by `$omegawiki-init` (a `raw/tmp/...` path for user-owned papers, or a `raw/discovered/...` path for introduced papers)
- `raw/` is strictly read-only — do not write to `raw/tmp/`, `raw/discovered/`, or anywhere else under `raw/`
- `fetch_s2.py citations <arxiv-id>` and `fetch_s2.py references <arxiv-id>` are **skipped** — the parent `$omegawiki-init` does a unified citation sweep at fan-in
- `rebuild-context-brief` and `rebuild-open-questions` are **skipped** — the parent runs them once after all subagents merge
- conflict-prone topic writes are **skipped** — if multiple parallel ingests all try to append to the same topic, they will merge-conflict. Let the parent handle topic updates after fan-in, or defer them to `$omegawiki-edit`.
- **skip reverse-link edits to existing pages** — do not append `key_papers` to an existing concept page, do not append to `## Key papers` or `## Related` of an existing paper page, and do not append to an existing people page. Record the relationship via `tools/research_wiki.py add-edge` instead. The parent `$omegawiki-init` rebuilds these backlinks during fan-in.
...
`$omegawiki-init` passes the canonical path in the subagent prompt. A `$omegawiki-ingest` invocation can recognize INIT MODE by either of:
...
Even outside INIT MODE, assume another `$omegawiki-ingest` may be running concurrently — batch ingest is already on the roadmap. Three rules make concurrent writes safe:
...
When two sibling `$omegawiki-ingest` subagents both need a new concept page with the same slug, both will try to create it and the fan-in merge will fail. Mitigations:

- the per-paper creation limit (`references/dedup-policy.md`) keeps the collision surface small
- the `$omegawiki-init` parent merges worktree branches sequentially; when the second worktree's ingest writes the same slug, the sequential merge resolves it as a conflict that the parent handles by picking the earlier write and re-running `find-similar-concept` on the later one at fan-in
...
## What `$omegawiki-ingest` does not do for `$omegawiki-init`

- It does not stash or switch branches.
- It does not merge worktrees or run `dedup-edges`, `rebuild-index`, or `lint.py --fix`. Those are fan-in operations owned by `$omegawiki-init`.

In INIT MODE, `$omegawiki-ingest` **must** commit its work inside the worktree before exiting, but only when the ingest completed successfully:
...
- do not push; the parent `$omegawiki-init` will merge the branch during fan-in

If the ingest fails part-way through (partial failure), do **not** commit the incomplete state. Let the parent `$omegawiki-init` handle the failed worktree at fan-in.
