# $omegawiki-ingest Error Handling

Open this reference when a step fails. `$omegawiki-ingest` prefers to degrade gracefully: record what happened, continue with what remains, and surface the gap in the final report.
...
- **INIT MODE input unreadable**: do not attempt to re-prepare the source (INIT MODE is read-only on `raw/`). Stop, record the failure, and let the parent `$omegawiki-init` retry or skip the paper at fan-in.
...
Then retry `$omegawiki-ingest`. Do not attempt to create pages in a non-initialized wiki; `index.md` and `graph/` scaffolding must exist first.
...
- surface the incomplete steps in the user report so the user can run `$omegawiki-edit` or `$omegawiki-check --fix` to finish the job
- in INIT MODE, if the ingest completed successfully, commit inside the worktree before exiting (see `references/init-mode.md`). If the ingest partially failed, do **not** commit the incomplete state; let the parent `$omegawiki-init` handle the failed worktree at fan-in
...
The guiding principle: a partial ingest that preserves a well-shaped paper page is more useful than a clean abort that leaves the wiki unchanged. Partial state is recoverable via `$omegawiki-check` and `$omegawiki-edit`. Lost partial state is not.
