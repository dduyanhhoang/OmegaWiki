...
> Each round calls $omegawiki-review for structured feedback → parses actionable items → Codex fixes the artifact →
...
- `--difficulty` (optional, default `hard`): difficulty level passed to $omegawiki-review
- `--focus` (optional): review focus passed to $omegawiki-review
...
- `wiki/graph/context_brief.md` — global context passed to $omegawiki-review
...
**Category A — Method/content issues (Codex fixes directly):**
...
- Idea novelty argument too thin → suggest running `$omegawiki-novelty` rerun
- Method has no source_papers → flag for `$omegawiki-ingest` review
- Missing related work citations → suggest running `$omegawiki-ingest` to add papers
- Requires experimental validation → suggest running `$omegawiki-exp-run`
- → Record in `unresolved_issues`, list suggested operations in the report

**Category C — Idea / method updates (Codex fixes wiki):**
...
| 1 | Idea novelty argument too thin | major | Flagged [[idea-slug]] for `$omegawiki-novelty` rerun |
...
|-------|----------|------------------|
| Missing experimental validation | critical | Run `$omegawiki-exp-design {slug}` |
| Missing comparison paper | major | Run `$omegawiki-ingest` for {paper-title} |
...
- **Pass review parameters through**: --difficulty and --focus are passed through to $omegawiki-review; maintain consistent review standards
...
- **$omegawiki-review call fails**: retry once; if still failing, terminate the loop and output the improvement history completed so far
...
- `$omegawiki-review` — each round's review (core dependency)
...
### Shared References
- `.agents/skills/shared-references/cross-model-review.md` — indirect dependency via $omegawiki-review
