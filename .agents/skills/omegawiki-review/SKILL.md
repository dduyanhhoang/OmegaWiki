...
> Can be used standalone or called by $omegawiki-ideate, $omegawiki-refine, $omegawiki-exp-design.
...
- `--difficulty` (optional, default `standard`):
  ...
  - `hard`: multi-round dialogue (up to 3 rounds), Codex rebuts each weakness
...
- **None**. Review is a read-only query operation.
  - Review results are output to terminal; the user or caller (e.g. $omegawiki-refine) decides whether to apply them.
...
**Follow cross-model-review.md**: do not send any of Codex's pre-judgments to Review LLM.
...
1. Codex analyzes Review LLM's weaknesses and classifies each:
   - **Rebuttal**: Codex has strong reasoning or wiki evidence to counter it → write a rebuttal
...
2. Send Codex's response to Review LLM:
   ```
...
|--------|--------|-------|------------------|
| [[idea-slug]] | novelty_score 2/5 | Novelty argument is thin | Run $omegawiki-novelty rerun |
| [[method-slug]] | source_papers sparse | Missing source paper backing | Ingest the missing paper, then rerun $omegawiki-check |

### Knowledge gaps identified
| Gap | Related to | Suggested action |
|-----|-----------|------------------|
| {description} | [[slug]] | $omegawiki-ingest, $omegawiki-exp-run, or /query |
...
**Codex**: {summary of rebuttals/acknowledgments}
...
- **Reviewer independence**: strictly follow `shared-references/cross-model-review.md`; do not leak Codex's pre-judgments to Review LLM
- **Do not modify wiki**: review only outputs suggestions; it does not directly modify any wiki pages. Wiki modifications are handled by the caller (e.g. $omegawiki-refine)
...
- **Review LLM unavailable**: downgrade to Codex self-review mode; annotate report with "single-model review, cross-model verification unavailable"; recommend the user retry with Review LLM later
...
- **Review LLM returns invalid response**: retry once; if still invalid, use Codex self-review fallback
...
### Shared References
- `.agents/skills/shared-references/cross-model-review.md` — reviewer independence principle (required reading)

### Called by
- `$omegawiki-ideate` Phase 4 (review top ideas)
- `$omegawiki-refine` each iteration round (review current version)
- `$omegawiki-exp-design --review` (review experiment plan)
