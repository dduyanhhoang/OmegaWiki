---
name: omegawiki-discover
description: Build a ranked shortlist of candidate papers (anchor-driven, topic-driven, or derived from current wiki state) that the user — or an upstream skill — may decide to feed into `$omegawiki-ingest`. Use whenever the user asks "what should I read next", "find papers similar to this one", "recommend related work", "what's around this topic", or whenever `$omegawiki-ingest` is invoked with `--discover`. Does not ingest; only proposes.
argument-hint: "(--anchor <id> [--anchor <id>] [--negative <id>] | --topic <str> | --from-wiki | --venue <slug> --year <int>) [--limit N]"
---

# $omegawiki-discover

> Produce a ranked shortlist of paper candidates from one of four seed modes. Surface them to the user (or to the calling skill) with rationales. Never auto-ingest — `$omegawiki-discover` is a proposal stage, `$omegawiki-ingest` is the action stage.
...
- `references/ranking-signals.md` — what `tools/discover.py` scores on and why discovery does **not** share `$omegawiki-init`'s survey preference
- `references/wiki-dedup.md` — how candidates are filtered against `wiki/papers/` and what to do with matches

## Inputs

- `--anchor <id>` (repeatable): one or more anchor paper IDs (arXiv IDs preferred; S2 paperIds also accepted). Drives the **anchor mode** — the primary use case, including the post-`$omegawiki-ingest` "what to read next" flow.
- `--negative <id>` (repeatable, optional): IDs to push recommendations away from. Only meaningful with `--anchor`.
- `--topic "<str>"`: a topic / query string. Drives the **topic mode** — lighter alternative to `$omegawiki-init`'s planner.
...
`$omegawiki-discover` does not write anywhere else in `wiki/` and does not touch `raw/`. `from-venue` is stricter: it does not write to `wiki/` at all, including `wiki/log.md`. Whether to actually pull a candidate into the wiki is the caller's decision (a follow-up `$omegawiki-ingest`).
...
- none. Graph mutations belong to `$omegawiki-ingest`, not `$omegawiki-discover`.
...
- the user named one or more specific papers, or this is a post-`$omegawiki-ingest` `--discover` follow-up → **anchors**
...
```
To ingest a candidate: $omegawiki-ingest https://arxiv.org/abs/<arxiv-id>
```
...
`$omegawiki-discover` is designed to be invoked both by users (manually) and by other skills (as a subroutine).

### From `$omegawiki-ingest --discover`

When `$omegawiki-ingest` is invoked with the optional `--discover` flag (default off), it calls `$omegawiki-discover` after the final report, with the just-ingested paper's arXiv ID as the single anchor. The shortlist is appended to `$omegawiki-ingest`'s report under a "Related papers you may want to ingest next" heading. `$omegawiki-ingest` never auto-ingests anything from this list.

### From `$omegawiki-init`

`$omegawiki-init` does not call `$omegawiki-discover`. `$omegawiki-init`'s planner (`tools/init_discovery.py plan`) has its own scoring that favors surveys, broad coverage, and seed anchors — appropriate for bootstrapping a wiki. `$omegawiki-discover`'s ranking is intentionally different (no survey preference; weights anchor similarity and influential citations) and would dilute `$omegawiki-init`'s shortlist if substituted in. Keep them separate.

## Constraints

- **Never auto-ingest**: `$omegawiki-discover` returns a shortlist and stops. Even when called by `$omegawiki-ingest --discover`, the caller surfaces results and the user decides what to ingest.
- **No content writes to `wiki/`**: paper pages, concepts, methods, graph edges all belong to `$omegawiki-ingest`. Anchor/topic/wiki runs may append `wiki/log.md`; `from-venue` must not write to `wiki/` at all.
- **No writes to `raw/`**: `$omegawiki-discover` does not download papers. The user runs `$omegawiki-ingest <arxiv-url>` afterwards if they want a candidate.
...
- **Ranking is discovery-specific**: do not import or duplicate `tools/init_discovery.py`'s scoring helpers. The two skills have different objectives — `$omegawiki-init` wants broad foundational coverage; `$omegawiki-discover` wants relevant *next reads*. See `references/ranking-signals.md`.
- **Three-channel anchor gather**: by default, anchor mode pulls from S2 `recommend` + `references` + `citations` per anchor. Removing the citation channels (via `--no-citation-expand`) collapses the result into a recency-biased semantic cluster that overlaps heavily with `$omegawiki-daily-arxiv`. Keep all three on unless API cost is a hard constraint. See `references/ranking-signals.md`.
...
- `$omegawiki-ingest` — caller via `--discover` flag; also the action the user takes on a chosen candidate
- `$omegawiki-init` — independent planner; does not call `$omegawiki-discover`
