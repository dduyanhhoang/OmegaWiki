...
Use when the user named one or more specific papers, or when this is a post-`$omegawiki-ingest` `--discover` follow-up.
...
- `$omegawiki-ingest --discover` invocation (anchor = the just-ingested paper's arXiv ID)
...
Topic mode runs S2 search and (when available) DeepXiv search, then ranks. It is a lighter alternative to `$omegawiki-init`'s planner: useful for exploration but **not** a replacement for `$omegawiki-init`'s broader bootstrap workflow. If the user wants to seed a fresh wiki with a topic, route them to `$omegawiki-init` instead of bulking up `$omegawiki-discover`.
...
If `wiki/papers/` is empty or no papers carry an `arxiv` or `arxiv_id` frontmatter field, wiki mode cannot run. Tell the user the wiki is too sparse and suggest topic mode (or `$omegawiki-init`).
