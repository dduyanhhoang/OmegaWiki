...
Manual: `$omegawiki-reset --scope wiki` / `--scope raw` / `--scope log` / `--scope checkpoints` / `--scope all`. Multiple scopes may be combined comma-separated: `--scope wiki,log`.
...
If the user says no, exit. **Never proceed without explicit approval** — `$omegawiki-reset` is destructive and `raw/` deletions are not tracked by git.
...
Next steps:
- $omegawiki-init       — bootstrap wiki from raw/
- $omegawiki-prefill    — seed foundational background
- $omegawiki-ingest     — add a single source manually
```

## Constraints

- **Confirm before destructive action**: never call `--yes` without showing the plan and asking the user.
- **Preserves**: `.gitkeep` placeholders, `wiki/AGENTS.md`, `.agents/` (skills are never touched).
- **`raw/` deletes are irreversible**: PDFs are not in git history. Warn the user before executing `raw` or `all` scopes.
- **`$omegawiki-reset` does not touch `tools/`, `mcp-servers/`, `i18n/`, `.env`, or git state.**
- **Scope is required**: no default action (`$omegawiki-reset` with no flag prompts for scope rather than guessing).

## Error Handling

- **Unknown scope**: print valid scopes and exit nonzero.
- **Missing wiki directory**: report and suggest running `$omegawiki-init`.
...
- `reset_wiki.py` clears `wiki/.checkpoints/*.json` directly for `checkpoints` scope (no CLI dispatch — the `checkpoint-clear` subcommand requires a specific `task_id`, while `$omegawiki-reset --scope checkpoints` semantics is "clear everything")
