...
- `.agents/skills/shared-references/academic-writing.md` — writing principles
- `.agents/skills/shared-references/citation-verification.md` — citation discipline
...
- If any target idea has empty `novelty_score` OR `novelty_score <= 2`: warn "idea novelty is thin; consider running `$omegawiki-novelty` first"
...
6. **Print PAPER_PLAN_REPORT to terminal**:
   ...
   ## Next Steps
   - Run `$omegawiki-paper-draft wiki/outputs/paper-plan-{slug}-{date}.md` to draft the paper
   - Resolve {verify_count} [UNCONFIRMED] citations before $omegawiki-paper-compile
   ```
...
- **No experiment evidence**: error "at least one experimental result is required"; suggest running $omegawiki-exp-design + $omegawiki-exp-run first
- **Insufficient wiki papers**: if the citation plan has fewer than 5 wiki papers, warn "related work coverage is insufficient; consider $omegawiki-ingest of more papers first"
- **Page budget exceeded**: automatically move lower-priority sections to appendix plan; report the adjustment
- **Review LLM unavailable**: fall back to Codex self-review; report annotated "single-model review — cross-model verification unavailable"
...
### Shared References
- `.agents/skills/shared-references/academic-writing.md` — narrative structure and section design principles
- `.agents/skills/shared-references/citation-verification.md` — citation fetch and verification rules

### Called by
- `$omegawiki-research` Stage 5 (paper writing stage)
