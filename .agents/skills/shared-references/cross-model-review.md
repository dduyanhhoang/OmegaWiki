...
> Referenced by: `$omegawiki-review`, `$omegawiki-novelty`, `$omegawiki-ideate`, `$omegawiki-exp-eval`, `$omegawiki-exp-design`, `$omegawiki-paper-plan`, `$omegawiki-paper-draft`, `$omegawiki-rebuttal`, `$omegawiki-refine`
...
- Codex's own score or rating of the artifact
- Codex's assessment of strengths/weaknesses
- Codex's recommendation (proceed/modify/abandon)
...
1. **Anchoring bias**: If the Review LLM sees "Codex rated this 7/10", its review will cluster around 7. Independent assessment catches blind spots that anchored assessment misses.
2. **Confirmation bias**: If Codex says "the main weakness is X", the Review LLM will focus on X and miss weakness Y. Unprimed reviewers explore the full space.
...
### In `$omegawiki-review` (adversarial critique)
- Step 2: Send artifact + context + review prompt to the Review LLM. Do NOT include any pre-assessment.
- Step 3 (multi-turn): Codex may respond to the Review LLM's critique with rebuttals, but these are responses to its points, not pre-formed judgments.

### In `$omegawiki-novelty` (cross-verification)
- Step 3: Send method signature + existing similar works to the Review LLM. Do NOT include Codex's novelty score from Step 2.

### In `$omegawiki-ideate` (dual-model brainstorm)
- Phase 2: The Review LLM generates ideas from the same landscape context as Codex, but does NOT see Codex's idea list. Merge happens after both complete independently.

### In `$omegawiki-exp-eval` (impartial verdict)
- Step 2: Send experiment results + the linked idea's hypothesis + context to the Review LLM. Do NOT include Codex's interpretation of the results.
...
1. **Do NOT silently skip the review step.** Inform the user:
   > "Cross-model review is not configured. This skill works best with an independent review LLM. Would you like to set it up now, or proceed with Codex-only analysis?"
...
2. **If the user wants to configure**, guide them interactively:
   ...
   - Tell them to restart Codex so the MCP server picks up the new config
   - Reference `.env.example` for the full provider table

3. **If the user wants to proceed without review**, continue with Codex-only mode:
   - Skip the `mcp__llm-review__chat` call
   - Perform the review/critique step using Codex itself (self-review)
   - Clearly mark the output as `[Codex self-review — no independent second opinion]`
