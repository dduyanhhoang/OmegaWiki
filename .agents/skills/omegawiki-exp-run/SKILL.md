...
>
> Recommended flow: `$omegawiki-exp-run <slug>` to deploy → `$omegawiki-exp-status` to monitor → `$omegawiki-exp-run <slug> --collect` to collect.
...
### Graph edges created
- **None**. The tested_by edges between experiments and ideas are created by $omegawiki-exp-design.
...
5. **Launch remote experiment**:
   ...
   python3 tools/remote.py launch \
     ...
     --gpu {gpu_index}
   ```
6. Update `wiki/experiments/{slug}.md` frontmatter — all of these fields already exist (empty) because `$omegawiki-exp-design` wrote the full AGENTS.md template:
   ```bash
   ...
   remote:
     ...
     completed: ""
   ```
   Use five Edit calls (one per sub-field) to set `server`, `gpu`, `session`, `started`. Leave `completed: ""` — Phase 4 fills that. If you find the `remote:` block missing from the file, that means `$omegawiki-exp-design` did not write the full AGENTS.md template; stop and report the bug rather than trying to append the block here (appending would drift the file away from the canonical order and break future edits).
...
1. Monitor progress: `$omegawiki-exp-status`
2. Check this experiment: `$omegawiki-exp-run {slug} --collect`
3. In $omegawiki-research pipeline: progress saved to wiki/outputs/pipeline-progress.md
...
3. **If experiment is still running (alive == true)**:
   ...
   - **Print progress report** (do not modify wiki, report only):
     ...
     Run `$omegawiki-exp-status` to monitor all running experiments.
     ```
...
6. **Print RUN_REPORT to terminal**:
   ...
   ## Next Steps
   - Run `$omegawiki-exp-eval {slug}` to update the linked idea in wiki
   - {if succeeded: proceed to next experiment in plan}
   - {if failed: analyze failure, consider $omegawiki-exp-design revision}
   ```
...
- **Do not update the linked idea's status**: experiment results are written only to experiments/ pages; idea updates are handled by $omegawiki-exp-eval
...
- **Graph edges are not created here**: tested_by edges were created by $omegawiki-exp-design
...
- **Deploy mode but status == running**: prompt "already running — use `$omegawiki-exp-run {slug} --collect` to check status"
- **Collect mode but status == completed**: prompt "already completed — run `$omegawiki-exp-eval {slug}` directly"
...
### Called by
- `$omegawiki-research` Stage 3a (deploy mode) and Stage 3c (collect mode)
- `$omegawiki-exp-status --collect-ready` (collect mode)
- User directly
