...
- `--auto` (optional): automatic mode — do not pause for user confirmation before wiki updates (used when called by $omegawiki-research)
...
- `.agents/skills/shared-references/cross-model-review.md` — reviewer independence principle
...
**Follow cross-model-review.md**: do not send Codex's pre-judgment to Review LLM.
...
### Step 3: Codex Synthesis

1. **Form Codex's independent verdict** (after reading Review LLM's verdict, Codex also analyzes independently):
   - Based on experimental results, the idea's hypothesis, and aggregate evidence from sibling experiments
   - Form Codex's own verdict and idea-status recommendation
...
3. **Suggest next steps**: `$omegawiki-paper-plan {linked-idea}` or continue ablation/robustness experiments
...
3. **Suggest supplementary experiments**:
   ...
   - Suggest using `$omegawiki-exp-design --linked-idea {linked-idea}` to design supplementary experiments
...
3. **Suggest next steps**:
   ...
   - Suggest `$omegawiki-ideate` to generate alternatives

#### Path D: INCONCLUSIVE (results are uncertain)

1. **Do not modify idea status**: insufficient evidence to make a judgment

2. **Update experiment page**: outcome is already inconclusive (set by $omegawiki-exp-run)
...
1. **Fill in the `## Idea updates` section of the experiment page** (records changes to the linked idea):
   ...
   - **Judge agreement**: {Codex and Review LLM agreed / disagreed on ...}
   - **Date**: YYYY-MM-DD
   ```
...
5. **Print VERDICT_REPORT to terminal**:
   ...
   ## Key Reasoning
   {2-3 sentences from Review LLM + Codex synthesis}
...
- **Only process completed experiments**: experiments with status != completed are refused; prompt user to use $omegawiki-exp-run first.
...
- **Reviewer independence**: strictly follow cross-model-review.md — do not send Codex's pre-judgment to Review LLM.
...
- **Conservative principle**: when Codex and Review LLM verdicts disagree, use the more conservative verdict.
...
- **Experiment not completed**: report status, suggest running `$omegawiki-exp-run {slug}` or `$omegawiki-exp-run {slug} --check`.
- **`linked_idea` missing**: refuse to proceed; instruct the user to run `$omegawiki-edit` to set the experiment's `linked_idea`.
- **Linked idea page does not exist**: report a dangling reference; refuse to update — recommend `$omegawiki-edit` or `$omegawiki-ideate` to create the idea page first.
- **Review LLM unavailable**: fall back to Codex single-model verdict, note "single-model verdict, cross-model verification unavailable" in report, suggest user confirm later.
...
- **Results data missing**: if the experiment page's Results section is empty, prompt user to run `$omegawiki-exp-run {slug} --check` first.
...
### Shared References
- `.agents/skills/shared-references/cross-model-review.md` — Review LLM independence principle (required reading)

### Called by
- `$omegawiki-research` Stage 4 (verdict and iteration stage)
- User directly
