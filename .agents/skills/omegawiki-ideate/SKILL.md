...
> Phase 1 scans the research landscape (wiki + WebSearch + S2), Phase 2 runs a dual-model brainstorm (Codex + Review LLM independently),
> Phase 3 applies a first-pass filter (feasibility + quick novelty check), Phase 4 performs deep validation (calls $omegawiki-novelty + $omegawiki-review),
...
- `--auto`: fully automatic mode, no pause for user confirmation (used when called by $omegawiki-research)
...
Goal: generate ideas independently with Codex and Review LLM, exploiting the diversity that comes from different model perspectives.

**Follow `shared-references/cross-model-review.md`**: Codex and Review LLM generate independently without seeing each other's output.

1. **Codex generates 6–10 ideas**:
...
3. **Merge and deduplicate**:
   - Combine Codex's and Review LLM's ideas (10–16 candidates)
...
1. **Call $omegawiki-novelty `--write`** (one at a time):
   ```
   For each top idea:
   Skill: novelty
...
2. **Call $omegawiki-review** (for top 2 ideas):
   ```
   Skill: review
...
1. **Write top ideas** (status: proposed):
   ...
   novelty_score: ""         # 1-5 — written by $omegawiki-novelty --write in Phase 4; leave empty otherwise
   priority: 3               # 1-5 — see Priority computation below
   pilot_result: ""          # empty until $omegawiki-exp-eval fills it
   failure_reason: ""        # empty for proposed ideas
   linked_experiments: []    # empty until $omegawiki-exp-design creates experiments
   ...
   **Priority computation** (maps Phase 4 signals into the 1-5 scale):
   ...
   - Otherwise start from `novelty_score` (1-5 from $omegawiki-novelty)
   ...
   **Body sections** (exactly match `runtime/templates/ideas.md.tmpl` — do not rename):
   ...
   ## Novelty argument
   Why this idea is genuinely new — what closest prior work (from $omegawiki-novelty) it differs from, and on which axis. One short paragraph.

   ## Target venue
   The intended publication target (e.g. NeurIPS 2026 / ICLR / ICML / ACL / COLM). May be left blank for ideas still being scoped.

   ## Risks
   Feasibility rating (high/medium/low) + top 2-3 risks. Include the main weaknesses surfaced by $omegawiki-review.

   ## Pilot results
   (empty — filled by $omegawiki-exp-eval after running the experiment)

   ## Lessons learned
   (empty — filled by $omegawiki-exp-eval after the idea reaches a terminal status)
   ```
...
2. **Write eliminated ideas** (status: failed):
   ...
   - `failure_reason: "[filter] <specific elimination reason>"` — the `[filter]` prefix distinguishes ideate-stage eliminations from post-experiment failures (which $omegawiki-exp-eval tags differently). Examples: `"[filter] highly similar published work exists: <paper-title>"`, `"[filter] insufficient feasibility: GPU requirements too high"`
...
6. **Print IDEA_REPORT to terminal**:
   ...
   - Phase 2: Generated {X} candidates (Codex: {a}, Review LLM: {b})
   ...
   - Run `$omegawiki-exp-design {top-idea-slug}` to design experiments
   - Run `$omegawiki-novelty` on any idea before investing time
...
- **Review LLM independence**: in Phase 2, Review LLM does not see Codex's idea list (cross-model-review.md)
...
- **Review LLM unavailable**: Phase 2 uses Codex only (no dual-model diversity, noted in report)
- **$omegawiki-novelty fails**: if novelty fails for a single idea in Phase 4, mark "novelty unverified" and continue
- **$omegawiki-review fails**: if review fails in Phase 4, mark "unreviewed" and continue; recommend user manually runs $omegawiki-review
- **Slug conflict**: if the same slug already exists in wiki/ideas/, append a numeric suffix (e.g. `sparse-lora-v2`)
- **All ideas eliminated**: still write to wiki (status: failed); report recommends user broaden the search direction or $omegawiki-ingest more papers
...
- `$omegawiki-novelty` — Phase 4 deep novelty validation
- `$omegawiki-review` — Phase 4 cross-model review
...
### Shared References
- `.agents/skills/shared-references/cross-model-review.md` — Phase 2 Review LLM independence principle
