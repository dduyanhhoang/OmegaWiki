...
Manual: `$omegawiki-prefill [domain]` or `$omegawiki-prefill --add "concept name"`.
...
- `.agents/skills/prefill/foundations-catalog.yaml` — seed list
...
**Pre-conditions**: working directory contains `wiki/`, `tools/`, `.agents/`. Set `WIKI_ROOT=wiki/`.
...
- **Catalog mode**: read `.agents/skills/prefill/foundations-catalog.yaml`. Pick all entries under `domains.{domain}` plus everything under `domains.general` (general foundations apply to every research field).
...
Remind the user that subsequent `$omegawiki-ingest` runs will dedup against these foundations and create wikilinks (`[[foundation-slug]]`) instead of new concept pages.
...
- **Catalog file missing**: print error pointing to `.agents/skills/prefill/foundations-catalog.yaml`.
...
### Catalog
- `.agents/skills/prefill/foundations-catalog.yaml`
