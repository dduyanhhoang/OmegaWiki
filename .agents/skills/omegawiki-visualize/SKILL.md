...
exists yet and suggest running `$omegawiki-ingest` first.
...
SPA Reader view. `$omegawiki-visualize` no longer regenerates `wiki/graph-view.html`.
...
```markdown
## [YYYY-MM-DD] $omegawiki-visualize | <format> — <n> nodes, <m> edges<focus-note>
```
...
- `wiki/.obsidian/` and `wiki/canvases/` are gitignored as local-only artifacts; the source of truth is `config/visualize.json` + `wiki/graph/`. `$omegawiki-init` Step 6 and direct `$omegawiki-visualize` invocations regenerate them deterministically — never commit them.

## Error Handling

- **No graph data**: inform user to run `$omegawiki-ingest` first to build the knowledge base
