...
> Build a wiki from `raw/` with deterministic source preparation, planner-guided discovery, provisional notes/web scaffolding, and parallel `$omegawiki-ingest` fan-out/fan-in.
...
- Final paper pages via parallel `$omegawiki-ingest` subagents
...
- `$omegawiki-init` itself creates only scaffold-level edges when provisional pages need them
- all paper-driven edges are delegated to `$omegawiki-ingest`

## Workflow

**Pre-condition**: working directory is the project root containing `wiki/`, `raw/`, and `tools/`. Set `WIKI_ROOT=wiki/`. Resolve `PYTHON_BIN` once and reuse it for every Python command during `$omegawiki-init` so the workflow stays on the interpreter that `setup-codex.sh` prepared:
...
- keep notes/web on their original source paths; `$omegawiki-init` reads them directly during planning
...
- record warnings for failed decode / title recovery / arXiv source fetch rather than aborting `$omegawiki-init`
...
- external papers downloaded by `$omegawiki-init` go to `raw/discovered/`, never `raw/papers/`
...
```markdown
Provisional note: seeded from raw/notes or raw/web during $omegawiki-init; pending validation from ingested papers.
```
...
- `methods/`: do not create from `$omegawiki-init` unless the user explicitly names a reusable, citable method in notes/web; ingest is responsible for promoting paper methods into reusable method entities
- `$omegawiki-prefill` is optional background seeding and is not part of `$omegawiki-init`
- `$omegawiki-init` must not create `people/` pages directly and must not auto-create foundations
...
- `$omegawiki-init` worktree mode must run from a named branch, not detached HEAD
...
- execute `$omegawiki-ingest` for exactly one handed-off source path; do not bypass `$omegawiki-ingest`
...
"$PYTHON_BIN" tools/visualize.py generate-obsidian-config wiki/ \
  || echo "WARN: visualize generate-obsidian-config failed; run $omegawiki-visualize manually" >&2
...
- pages created by `$omegawiki-ingest`
- pages updated by `$omegawiki-ingest`
...
- `raw/tmp/` and `raw/discovered/` are generated handoff areas; direct local `$omegawiki-ingest` may also prepare reusable local sidecars under `raw/tmp/`
- `$omegawiki-init` may write external papers only to `raw/discovered/`; `$omegawiki-init` and direct local `$omegawiki-ingest` may write generated prepared local sources to `raw/tmp/`
- `$omegawiki-prefill` is optional background seeding, not part of `$omegawiki-init`
- no skill other than `$omegawiki-prefill` may auto-create foundations
- `$omegawiki-init` must not create `people/` pages directly
...
- all paper ingest must run through parallel `$omegawiki-ingest` subagents with worktree isolation
...
- **Visualization regeneration fails**: warn and continue; never fail `$omegawiki-init`. The user can rerun `$omegawiki-visualize --canvas --html` separately to diagnose
...
- `$omegawiki-ingest` — one paper per subagent, in INIT MODE
- `$omegawiki-visualize` — Step 6 fan-in regenerates Obsidian graph color groups, Canvas, and HTML by calling `tools/visualize.py` directly (best-effort); the user may also invoke `$omegawiki-visualize` manually later for `--focus` views or to re-render after editing `config/visualize.json`
