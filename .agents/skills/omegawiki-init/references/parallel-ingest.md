# $omegawiki-init Parallel Ingest

Use this reference when `$omegawiki-init` is handing sources to parallel `$omegawiki-ingest` subagents and merging their work back.
...
- `$omegawiki-init` worktree mode requires a named branch; stop on detached HEAD.
...
- Execute `$omegawiki-ingest` for exactly one relative source path.
- Do not bypass `$omegawiki-ingest`.
